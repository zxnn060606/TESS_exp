...
class PastDecomposableMixing(nn.Module):
    def __init__(self, configs):
        ...
        self.layer_norm = nn.LayerNorm(configs['embedding_size'])
        ...
        if not configs['channel_independence']:
            self.cross_layer = nn.Sequential(
                nn.Linear(in_features=configs['embedding_size'], out_features=configs['embedding_size']),
                nn.GELU(),
                nn.Linear(in_features=configs['embedding_size'], out_features=configs['embedding_size']),
            )
...
class TimeMixer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.channel_independence:
            self.enc_embedding = DataEmbedding_wo_pos(1, configs['embedding_size'], configs['embed'], configs['freq'],
                                                    configs['dropout'])
        else:
        ...
        if self.channel_independence:
            self.projection_layer = nn.Linear(
                configs['embedding_size'], 1, bias=True)
        else:
            self.projection_layer = nn.Linear(
