...
class Encoder(nn.Module):
    ...
    def __init__(self, configs, window_size, inner_size):
        ...
        d_bottleneck = configs['embedding_size']//4
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(configs['embedding_size'], configs['embedding_size'], configs['n_heads'], dropout=configs['dropout'],
                        normalize_before=False) for _ in range(configs['e_layers'])
        ])  # naive pyramid attention

        self.enc_embedding = DataEmbedding(
            configs['enc_in'], configs['embedding_size'], configs['dropout'])
        self.conv_layers = Bottleneck_Construct(
            configs['embedding_size'], window_size, d_bottleneck)
